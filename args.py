...
from dataset.UniASET_constants import TASKS_GROUP_NAMES, TASKS_GROUP_TEST
...
parser.add_argument('--task_id', '-fold', type=int, default=None, choices=[0, 1, 2, 3])
...
# optional arguments
parser.add_argument('--model', type=str, default=None, choices=['UniAP'])
...
parser.add_argument('--sem_threshold', '-sth', type=float, default=None)
...
if config.exp_name == '':
    if config.stage == 0:
        if config.task == '':
            config.exp_name = f'{config.model}_fold:{config.task_id}{config.name_postfix}'
        else:
        ...
        for fold in TASKS_GROUP_TEST:
            for task in TASKS_GROUP_TEST[fold]:
                fold_dict[task] = fold
        task_id = fold_dict[config.task]
        config.exp_name = f'{config.model}_fold:{task_id}{config.name_postfix}'