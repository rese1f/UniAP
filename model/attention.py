...
class CrossAttention(nn.Module):
    ...
    def forward(self, Q, K, V, mask=None, get_attn_map=False, disconnect_self_image=False, H=None, W=None, **kwargs):
        ...
        # vis_L = L[1,:14:,:14:].float().cpu().detach()
        # normalized_vis_L = ((vis_L - vis_L.min()) / (vis_L.max() - vis_L.min()))
        # plt.imshow(normalized_vis_L, cmap='viridis')
        # plt.axis('off')
        # plt.savefig('heatmap.png',dpi=400, bbox_inches='tight', pad_inches=0)
