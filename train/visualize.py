...
from dataset.UniASET_constants import *
...
def visualize_batch(X=None, Y=None, M=None, Y_preds=None, task = None,channels=None, size=None, postprocess_fn=None, **kwargs):
    '''
    ...
    if task == 'keypoints2d':
        ...
        for i in range(Y_preds.size(0)):
            Y_preds_i = generate_gaussian_heatmap(Y_preds[i][0].cpu())            
            Y_preds[i][0] = torch.Tensor(Y_preds_i)
