...
def set_directories(config, root_dir='experiments', exp_name='', log_dir='logs', save_dir='checkpoints',
    ...
    if exp_name == '':
        if config.task == '':
            exp_name = config.exp_name = f'{config.model}_fold:{config.task_id}{config.name_postfix}'
        else:
...
def load_model(config, verbose=True):
    ...
    # create trainer for fine-tuning or evaluation
        ...
        ckpt_path = get_ckpt_path(config.load_dir, config.exp_name, -1)
        ...
        # load fine-tuned checkpoint
            ...
            # ft_ckpt_path = get_ckpt_path(config.save_dir, config.exp_name, -1, config.exp_subname, config.save_postfix)
